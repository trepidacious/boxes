...
class InvalidReactionException(s:String, r:Reaction, b:Box[_]) extends BoxException {

  override def toString = s + ":" + r

}

class InvalidReadException(b:Box[_]) extends BoxException {

  override def toString = "Invalid read of box " + b

}

class InvalidWriteException(b:Box[_]) extends BoxException {
